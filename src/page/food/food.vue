	    		...
	    		<transition name="showlist" v-show="category">
	    			<section v-show="sortBy == 'food'" class="category_container sort_detail_type">
	    				...
	    				<section class="category_left">
                            <ul>
                                <li class="category_left_li" style="line-height:45px;">全部商家</li>

                                <!-- index==0 时后台数据count  存在问题 -->
	    						<li v-for="(item, index) in category" :key="index" class="category_left_li" :class="{category_active:restaurant_category_id == item.id}" @click="selectCategoryName(item.id, index)">
									...
									<section>
	    								<span class="category_count">{{item.count}}</span><!--获取二级分类下分类的个数-->
    ...
    export default {
        ...
        methods: {
            ...
            async initData() {
              ...
              //获取category分类左侧数据  此处后台数据中档id==0是 name为全部商品 图标项为 空   点击时显示全部商品
              ...
              //获取筛选列表的商铺活动  商家属性  可以 多选
              this.Activity = await foodActivity(this.latitude, this.longitude);

            // console.log(JSON.stringify(this.Activity));

            //记录support_ids的状态，默认不选中，点击状态取反，status为true时为选中状态  商家属性是否选中的问题 status：boolean 值来切换选中状态
        ...
        sortList(event) {
            ...
            if (event.target.nodeName.toUpperCase() !== "P") {
                ...
                this.sortByType = node.getAttribute("data"); // 获取元素上绑定的data值来获取对应的筛选条件
                this.sortBy = "";
        },
        //筛选选项中的配送方式选择
