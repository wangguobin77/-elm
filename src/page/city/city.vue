    ...
    export default {
        ...
        methods:{
            ...
            postpois(){
                ...
                if (this.inputVaule) {
                    searchplace(this.cityid, this.inputVaule).then(res => {
                        ...
                        this.placelist = res;
                        console.log(res);
                        this.placeNone = res.length? false : true; //判断搜索列表中是否为空  为空为fasls 否则为true
                    })
                }
            },
            /**
            ...
            nextpage(index, geohash){
                let history = getStore('placeHistory'); //获取搜索框中的值
                let choosePlace = this.placelist[index];
                if (history) {//搜索框中的值不为空
                    let checkrepeat = false; //默认没有重复
                    ...
                    if (!checkrepeat) {//判断当前搜索值与已经搜索列表中的值是否含有重复  若非重复则追加一条信息  checkrepeat默认为false即没有重复信息
                        this.placeHistory.push(choosePlace)  //placehistory[]中压入当前信息
                    }
                }else {
