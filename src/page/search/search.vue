<template>
  	<div class="paddingTop search_page">
        ...
        <!-- 没有搜索结果时 判断显示   v-if='emptyResult'变量来判断 -->
...
export default {
    ...
    methods:{
        ...
        //fetch(url).then(function(){//处理转化为promise对象}).then(function(){})

//          async searchTarget(){
// 　　    　　
//         　　fetch("http://elm.cangdu.org/v4/restaurants")
//         　　
//         　　.then(res => {
//                 if(resp.status === 200){

//                     return res.json();//转化成了一个Promise对象，string 类型 JSON.stringify()
//                 }else{
...
//             .then(data=>{
//
//                 console.log(data);//处理为js对象{}  //返回的结果集 JSON.parse(data)
//             })
...
//         }
        ...
        // 点击删除按钮，删除当前历史记录  删除对应的搜索记录
        /*
            splice(index,last,'12','23','36')从index位开始删除last位，然后添加 '12' '23' '36'
         */
        deleteHistory(index){
            this.searchHistory.splice(index, 1);//从index为开始删除1位元素 即当前操作对象

            setStore('searchHistory',this.searchHistory);
        },
        //清除所有历史记录
        clearAllHistory(){
            this.searchHistory = [];//清空搜索记录中保存数组的值
           //执行函数 setStore()中 window.locatiom.setItem(name.content)
            setStore('searchHistory',this.searchHistory);
        }
    }
}
